...
class Blockchain {
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.validateChain().then(errors => typeof errors === 'string' ?  console.log(errors) : errors.forEach(error => console.error(error)));
            return new Promise(async (resolve, reject) => {
                let ownedBlocks = self.chain.filter(block => block.owner === address);
                if (ownedBlocks.length === 0) reject(new Error('Could not find address'));
                stars = ownedBlocks.map(block => JSON.parse(hex2ascii(block.body)));
                if (stars) {
                    resolve(stars);
                } else {
                    reject(new Error('Return stars failed!'));
                }
            });
        });
    }
